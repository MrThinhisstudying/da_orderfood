...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['updateProfilePhoto'])) {
        ...
        if($check !== false) {
            ...
            $uploaddir = $_SERVER['DOCUMENT_ROOT'].'/project_php_orderfood/img/';
            ...
            echo '<script>alert("Chọn ảnh để cập nhật");
            window.location=document.referrer;
                </script>';
        }
    }
    ...
    if(isset($_POST['removeProfilePhoto'])) {
        $id = $_POST["userId"];
        $filename = $_SERVER['DOCUMENT_ROOT']."/project_php_orderfood/img/person-".$id.".jpg";
        if (file_exists($filename)) {
            ...
            echo "<script>alert('Không tìm thấy ảnh để xóa');
                window.location=document.referrer;
            </script>";
        }
    }
